...
const numberToPromote = (corps, rank) => {
  switch (corps) {
    case constants.CORPS.KORPUS_STRZELCOW:
      if (rank === constants.RANKS.PLUTONOWY) return (number = "4");
      return (number = "3");
...
const show = async (interaction) => {
  ...
  if (!user) return await interaction.editReply("Podano niewłaściwego gracza.");
  ...
  if (userRecommendations === "") userRecommendations = "Brak";
  ...
  const myEmbed = {
    ...
    thumbnail: {
      url: `https://cdn.discordapp.com/avatars/${user.user.id}/${user.user.avatar}.jpeg`,
    },
    fields: [
      {
        name: "Stopień:",
        value: `${userData.payLoad.rankData.rank}`,
        inline: true,
      },
      {
        name: "Korpus",
        value: `${userData.payLoad.rankData.corps}`,
        inline: true,
      },
      { name: "\u200B", value: "\u200B" },
      ...
      {
        name: "Całkowita liczba rekomendacji",
        value: `${userData.payLoad.rankData.number}`,
      },
      {
        name: "Aktualna liczba ",
        value: `${userData.payLoad.rankData.currentNumber}`,
        inline: true,
      },
      {
        name: "Liczba do awansu",
        value: numberToPromote(
          userData.payLoad.rankData.corps,
          userData.payLoad.rankData.rank
        ),
        inline: true,
      },
      {
        name: "Gotowy do awansu",
        value: userData.payLoad.rankData.promotion ? "Tak" : "Nie",
      },
      {
        name: "Konto aktywne",
        value: userData.payLoad.accountActive ? "Tak" : "Nie",
      },
    ],
  };
